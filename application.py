...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        d = datetime.datetime.strptime(event_data['time'], "%Y-%m-%d %H:%M:%S")
        if not (d.month == billing_month or d.year == billing_date.year):
        ...
        if event_data['type'] == 'call':
            ...
            call_obj = Call(src_num, dst_num, date, duration,
                            event_data['src_loc'], event_data['dst_loc'])
            maker = find_customer_by_number(src_num, customer_list)
            receiver = find_customer_by_number(dst_num, customer_list)
            maker.make_call(call_obj)
            receiver.receive_call(call_obj)
