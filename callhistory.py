...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        time = call.time
        if (time.month, time.year) not in self.outgoing_calls:
            self.outgoing_calls[(time.month, time.year)] = []
            self.outgoing_calls[(time.month, time.year)].append(call)
        else:
            self.outgoing_calls[(time.month, time.year)].append(call)
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        """
        time = call.time
        if (time.month, time.year) not in self.incoming_calls:
            self.incoming_calls[(time.month, time.year)] = []
            self.incoming_calls[(time.month, time.year)].append(call)
        else:
            self.incoming_calls[(time.month, time.year)].append(call)
